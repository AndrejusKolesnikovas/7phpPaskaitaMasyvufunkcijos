...
function exercise3(): int
{
    ...
    */
    $cities = getCities();
    return array_reduce($cities,
        function (int $totalPopulation, array $city) {
...
function exercise4(): int
{
    ...
    */
    $city = getCities();
    $filtratedArray = array_filter($city, function (array $population) {
        if ($population['population'] > 25000000) {
            return true;
        }
    });

    return array_sum(array_column($filtratedArray, 'population'));
}
...
function exercise5(): array
{
    ...
    */
    ...
    return array_filter($city, function (array $population) {
        if ($population['population'] > 25000000) {
            return true;
        }
    });
...
function exercise6_foreach(): int
{
    ...
    */
    ...
    foreach ($orderItems as $item) {
        $price = in_array($item['name'], $lowPriceItems, true) ? $item['priceLow'] : $item['priceRegular'];
...
function exercise6_map(): int
{
    ...
    */
    ...
    $subtotals = array_map(
        function (array $item) use ($lowPriceItems): int {
            $price = in_array($item['name'], $lowPriceItems, true) ? $item['priceLow'] : $item['priceRegular'];
...
function exercise6_reduce(): int
{
    ...
    */
    ...
    return array_reduce(
        $orderItems,
        function (int $total, array $item) use ($lowPriceItems): int {
            $price = in_array($item['name'], $lowPriceItems, true) ? $item['priceLow'] : $item['priceRegular'];
